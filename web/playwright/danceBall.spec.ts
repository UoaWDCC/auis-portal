...
test("user1 members only single entry ticket", async ({ page }) => {
  ...
  await page.waitForTimeout(250);
  await page.getByRole("img", { name: "Dance Ball" }).click();
  await page
    .locator("div")
  ...
  await page
    .locator("div")
    .filter({ hasText: /^\$6\.99Get Tickets$/ })
    .getByRole("button")
    .click();
...
test("user1 members only double entry ticket", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Events" }).click();
  await page.waitForTimeout(250);
  await page.getByRole("heading", { name: "Dance Ball" }).click();
  await page
    .locator("div")
    .filter({ hasText: /^Get Tickets$/ })
    .getByRole("button")
  ...
  await page
    .locator("div")
    .filter({ hasText: /^\$10\.99Get Tickets$/ })
    .getByRole("button")
    .click();
